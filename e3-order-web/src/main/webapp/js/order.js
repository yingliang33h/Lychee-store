      ...
      $.ajax({
        ...
        success: function(msg){
          ...
          if(msg.status != 0) {
            ...
            divPayDataId.find('h3.orderTitle').html('支付及配送方式：<span class="addrAlter hide">[Change]</span><span class="orderTip">Please check the shipping address</span>');
  ...
  function hideShowEditHref() {
    if($(".orderCurr:hidden").length > 0 && $(".orderCurr:hidden").parent().parent().css("display") != 'none'){
      $(".addrAlter").each(function(){
        ...
        .html("Please save " + $(".orderCurr:hidden").parent().parent().find(".orderTitle").html().split("：")[0] +" before changing");
        }
      });
    } else {
