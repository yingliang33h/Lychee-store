...
function isOnline(home_url,passport_url){
	...
	$.getJSON( "/ajax/isOnline/?callback=?", function( data ) {
		if (data.welcome){
			...
			$('#login').html('<span class="logininfo"> '+data.welcome+'</span> <a href="'+passport_url+'/service/logout/?returnUrl='+passport_url+'/">[Logout]</a>'); 
		}else{
		...
		if(data.qqcb){
			$('#login').html('<span class="logininfo"> '+data.welcome+'</span> <a href="'+home_url+'/service/logout/?returnUrl='+passport_url+'/">[Logout]</a>'); 
			$('#qqcb').html(data.qqcb); 
		}
	});
}
			...
			for(i=0;i<5;i++){
				...
				if(temp){
					...
					if(temp.isStork){
						...
						$nowCon += '<div class="pImg"><a title="'+temp.title+'" target="_blank" href="'+temp.url + trackref + '"><img onerror="this.src=\'http://www.lycheeStore.net/html/images/150pic.jpg\'" alt="'+temp.title+'" src="'+temp.img+'"/></a></div>';
						...
						$nowCon += '<li class="tbover"><div><a title="'+temp.title+'" target="_blank" href="'+temp.url + trackref + '"><img onerror="this.src=\'http://www.lycheeStore.net/html/images/150pic.jpg\'" alt="'+temp.title+'" src="'+temp.img+'"/><span class="yover"></span></a></div>';
						$nowCon += '<div class="title-a"><a title="'+temp.title+'" target="_blank" href="'+temp.url+ trackref +'">'+temp.title+'</a></div>';
		...
		if(str.nextList.length<1){
			...
			for(i=0;i<5;i++){
				...
				if(temp){	
					styleId = temp.aid;
					$nextCon += "<li>";
					$nextCon += '<div class="pImg"><a title="'+temp.title+'" target="_blank" href="'+temp.url+trackref+'"><img onerror="this.src=\'http://www.lycheeStore.net/html/images/150pic.jpg\'" alt="'+temp.title+'" src="'+temp.img+'"></a></div>';
